(function() {
    ...
    function showRedirectFallback(redirectUrl) {
        if (!redirectUrl || document.getElementById('hcwc-redirect-fallback')) {
            return;
        }

        var banner = document.createElement('div');
        banner.id = 'hcwc-redirect-fallback';
        ...
        link.target = '_blank';
        link.rel = 'noopener';
        var brand = (window.kcGatewayData && window.kcGatewayData.brandName) || 'Payment';
        link.textContent = 'Continue to ' + brand + ' Payment';
        ...
        console.info('[HCWC] Automatic redirect failed. Showing fallback link:', redirectUrl);
    }
    ...
    function extractJsonFromResponse(text) {
        ...
        if (orderIdMatch) {
            try {
                ...
                console.warn('[HCWC] Method 1 failed:', e);
            }
        }
        ...
        if (lastBrace !== -1) {
            ...
            try {
                ...
                console.warn('[HCWC] Method 2 failed:', e);
            }
        }
        ...
        if (jsonStart !== -1) {
            try {
                ...
                console.warn('[HCWC] Method 3 failed:', e);
            }
        }
            ...
            if (url && url.indexOf('/wc/store/v1/checkout') !== -1 && response.ok) {
                response.clone().text().then(function(text) {
                    try {
                        ...
                        console.warn('[HCWC] Error processing checkout response:', e);
                    }
                }).catch(function() {
                    console.warn('[HCWC] Could not read checkout response body');
                });
            }
    ...
    OriginalXHR.prototype.open = function(method, url) {
        this._hcwcMonitor = url && url.toString().indexOf('/wc/store/v1/checkout') !== -1;
        return XHROpen.apply(this, arguments);
    };

    OriginalXHR.prototype.send = function(data) {
        if (this._hcwcMonitor) {
            this.addEventListener('readystatechange', function() {
                if (this.readyState === 4 && this.status >= 200 && this.status < 300) {
                    try {
                        ...
                        console.warn('[HCWC] Could not parse XHR checkout response:', e);
                    }
                }
            });
        }
    ...
    console.info('[HCWC] Enhanced redirect fallback system initialized (fetch + XHR monitoring)');
})();
